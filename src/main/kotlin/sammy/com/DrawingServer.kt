...
/*
* Manages the rooms and all the players inside a room
*
* */
class DrawingServer {
    ...
    fun playerLeft(clientId: String, immediatelyDisconnect:Boolean = false){
        ...
        if (immediatelyDisconnect){
            println("Closing the connection to ${players[clientId]?.username}")
    ...
    fun getRoomWithClientId(clientId: String): Room? {
        ...
        return if (filteredRooms.values.isEmpty()) null else filteredRooms.values.toList()[0]
    }
}